...
tags:
  ...
  - 图像处理
summary: >-
  文章针对发型图片换色需求，对比了SVG和Canvas两种方案的优劣，并重点调研了Canvas上实现类似渐变映射换色的方法。作者通过分析像素亮度与颜色的映射关系，提出了遍历像素点、将RGB转为HSL后按亮度区间替换颜色的实现思路，并给出了参考代码。同时指出该方案存在性能问题，后续计划采用tinygradient库优化渐变色生成，以实现更完整的渐变映射功能。
---
