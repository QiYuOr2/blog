...
其中 `weather.go` 封装了 [wttr.in](https://wttr.in) 的格式化输入输出，用来获取指定地区的天气情况；`tavily.go` 封装了 [tavily](https://www.tavily.com/) 的格式化输入输出，用来获取指定地区的旅行地点推荐，使用前需要提前去官网申请免费的 `apiKey`。这部分内容不是本文的重点，所以直接提供出来了。
