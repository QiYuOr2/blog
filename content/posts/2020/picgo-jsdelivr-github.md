...
summary: >-
  由于微博图床开启防盗链导致图片无法显示，文章提出使用 PicGo 配合 GitHub 仓库存储图片，并通过 jsDelivr
  进行加速访问的方案。具体操作包括新建 GitHub 仓库、生成个人访问 Token，以及在 PicGo 中配置仓库名、分支、Token
  和自定义域名。配置完成后，图片即可通过 jsDelivr 链接快速访问，解决了图床稳定性和速度问题。
---
