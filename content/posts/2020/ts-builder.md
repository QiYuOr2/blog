...
category: 技术
description: >-
  生成器模式是一种在TypeScript/JavaScript中非常常见的创建型设计模式，它使你能够分步骤创建复杂对象。当你需要创建一个可能有许多配置选项的对象时，
  该模式会特别有用。
summary: >-
  生成器模式通过将复杂对象的构造过程拆分为独立的步骤，避免了构造函数参数过多导致的可读性和灵活性差的问题。文中用 TypeScript 实现了
  `ProductBuilder`，通过链式调用 `setPartA` 等方法来分步配置产品属性，最终由 `build()`
  完成对象构建。该模式允许按需设置部分属性，简化了不同组合产品的创建过程，提升了代码的清晰度和可维护性。
---
