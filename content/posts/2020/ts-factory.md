...
summary: >-
  文章用TypeScript分别实现了简单工厂模式、工厂方法模式和抽象工厂模式，并通过披萨制作的示例代码展示了各自的结构与用法。简单工厂模式适用于业务简单场景，但扩展时需修改工厂类；工厂方法模式通过引入抽象工厂和具体工厂，新增产品时无需改动原有逻辑，符合开闭原则；抽象工厂模式则进一步引入产品族与产品等级结构概念，用于创建一组相关产品对象。
---
