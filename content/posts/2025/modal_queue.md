...
category: 技术
summary: >-
  这篇文章针对移动端多弹窗按优先级依次展示的需求，指出传统状态值或索引控制导致视图与业务耦合、难复用、难扩展的问题，提出基于队列的调度中心方案。通过
  `modalify` 统一封装弹窗接口，配合 `useModalQueue`
  管理弹窗入队与顺序弹出，并对比了两种入队方案，推荐更灵活、职责更纯粹的业务控制入队方案。最终实现解耦、可复用且易维护的多弹窗管理逻辑。
---
