...
category: 技术
description: >-
  作为一名程序员，写博客是积累知识、提升水平必不可少的一个方法。我们写博客主要有三种方法，一种是使用掘金、博客园、CSDN
  等博客网站，第二种是自己搭建网站，存放自己的博客，第三种就是使用静态博客生成器，将生成的网页部署到服务器或者 github pages、gitee
  pages 等服务上。这...
summary: >-
  作者通过对比常用博客方案，决定自研静态博客生成器 CoinRailgun，并详细介绍了其实现过程。文章围绕 init、new、build、server
  等核心命令，说明了依赖选择、项目结构、模板渲染及数据流处理思路。最终生成的静态站点能按设计的 URL
  规则组织页面，并以本地预览验证效果。整体提供了一个从需求分析到代码实现的完整参考案例。
---
