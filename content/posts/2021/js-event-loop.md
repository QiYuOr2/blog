...
category: 技术
description: >-
  事件循环机制是JavaScript运行的核心，因此要想写出bug少的js代码就必须要了解这个机制是如何运作的，以及为什么会有这个机制。本篇文章是我对Event
  Loop的理解
summary: >-
  JavaScript 因需操作 DOM
  而设计为单线程，但通过同步与异步任务划分解决阻塞问题，异步任务先进入任务队列，待执行栈清空后再读取执行。异步任务进一步分为微任务与宏任务，微任务优先级高于宏任务，在浏览器中每个宏任务执行后都会清空微任务队列，而
  Node.js 在 V11 后也遵循相同机制。理解这一顺序有助于编写更可靠的异步代码。
---
